...
export function ensureAnonymousSchema() {
  ready ??= db.execute(sql.raw(`ALTER TABLE donations ADD COLUMN IF NOT EXISTS poster_email text; ALTER TABLE donations ADD COLUMN IF NOT EXISTS poster_phone text; ALTER TABLE donations ADD COLUMN IF NOT EXISTS poster_contact_key text; ALTER TABLE donations ADD COLUMN IF NOT EXISTS image_url text; CREATE TABLE IF NOT EXISTS claims (id serial primary key, donation_id integer not null, claimant_email text, claimant_phone text, claimant_contact_key text not null, status text not null default 'claimed', created_at timestamptz not null default now()); CREATE TABLE IF NOT EXISTS point_events (id serial primary key, contact_key text not null, points integer not null, reason text not null, created_at timestamptz not null default now());`)).then(() => undefined)
  return ready
}
